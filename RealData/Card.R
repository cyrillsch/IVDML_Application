## R script to reproduce the tables and figures from Section 4.3.2
##################################################################
...
Y <- card.data$lwage
Z <- card.data$nearc4
X <- cbind(card.data$exper, card.data$black, card.data$smsa, card.data$smsa1966, card.data$south, card.data$reg661, card.data$reg662, card.data$reg663, card.data$reg664, card.data$reg665, card.data$reg666, card.data$reg667, card.data$reg668)
...
save(list_res, file = "CardResult.RData")

load(file = "CardResult.RData")
...
pdf("Plots/Card.pdf", width = 8, height = 6)
par(mfrow = c(2,2))
par(mgp = c(2, 1, 0))
par(bg = "white")
aa <- seq(0, 23, length.out = 80)
...
for(ml_method in c("gam", "xgboost")){
  ...
  for(h in c(h_normal, h_undersmooth)){
    ...
    plot(1, 1, type = "n", xlim = c(0, 23), ylim = c(-3, 3), 
         xlab = "experience", ylab = "beta", main = plot_title, cex.lab = 1.2)
